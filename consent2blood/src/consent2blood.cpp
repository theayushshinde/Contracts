...
checksum256 consent2blood::string_to_hash(const string &input)
{
  return sha256(input.c_str(), input.size());
}

ACTION consent2blood::consent(name user, name contract, checksum256 hash)
{
  require_auth(user);
...
ACTION consent2blood::revoke(name user, name contract)
{
  require_auth(user);
...
EOSIO_DISPATCH(consent2blood, (consent)(revoke)(clear))
